...
private const uint FILE_FLAG_NO_BUFFERING      =   0x20000000 ;
    ...
    public void UnpinBuffer()
    {
      ...
      Close();
      pHandle = CreateFile(FileName, GENERIC_WRITE, FILE_SHARE_WRITE, 0, CREATE_ALWAYS, FILE_FLAG_NO_BUFFERING | FILE_FLAG_WRITE_THROUGH | FILE_FLAG_SEQUENTIAL_SCAN, 0);
			if (pHandle == System.IntPtr.Zero)
			{
				Win32Exception WE = new Win32Exception();
      ...
      // use the ReadBlocks function below.
        ...
        public int Write(void* pointer, int BytesToWrite)
        {
            // Writes out the file in one swoop using the Windows WriteFile function.
            int NumberOfBytesWritten;
            if (!WriteFile(pHandle, pointer, BytesToWrite, &NumberOfBytesWritten, 0))
            {
                Win32Exception WE = new Win32Exception();
