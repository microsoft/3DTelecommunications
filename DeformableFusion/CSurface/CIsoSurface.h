...
/******************************************************************
 * Notice added by Mingsong Dou (doums@cs.unc.edu)
 * 1. the volumne data is saved in x-major oder: save value along x direction
 *    and then y direction, and finally z direction
 * 2. extend the code by allow null cells. Null cell is defined by IsNull function 
 *    (if the cell value falls in some range, it is null). 
 * 3. add color field to get color for surface
 ***************************************************************/
...
struct POINT3DID {
public:
	POINT3DID( unsigned int newID_=0,
			   float x_=0, float y_=0, float z_=0, 
			   float r_=-1, float g_=-1, float b_=-1)
			   :newID(newID_), x(x_), y(y_), z(z_), r(r_), g(g_), b(b_) {};
public:
	unsigned int newID;
	float x, y, z;
	float r, g, b;
...
public:
	...
	/* Note that ptScalarField is a buffer with the size (nCellsX + 1)*(nCellsY+1)*(nCellsZ+1)*/
	//Normals are calculated: looking from the normal direction, the triangle vertices are in couter clockwise order
	//                      : normals are along the descenting direction of the scalar field
...
protected:
	...
	//check if the edge is well defined (if either of the two end points of an edge is null, then the edge is null )
	bool IsEdgeNull(unsigned int nX, unsigned int nY, unsigned int nZ, unsigned int nEdgeNo, bool (*IsNull)(T val));
	...
	POINT3DID Interpolate(float fX1, float fY1, float fZ1, float fX2, float fY2, float fZ2, T tVal1, T tVal2, T const* clr1=NULL, T const* clr2=NULL);
