...
namespace Simplify
{
	...
	std::vector<vec3f> normals; // parallel size to vertices
	std::vector<Ref> refs;
	...
	void simplify_mesh(int target_count, double agressiveness=7, bool verbose=false)
	{
		...
		for (int iteration = 0; iteration < 100; iteration ++)
		{
			...
			std::ofstream myfile("D:\\meshSimplification.txt", std::ofstream::out | std::ofstream::app);
			myfile << "iteration - triangles - threshold %g\n" << iteration <<" " <<  triangle_count - deleted_triangles << " "<< threshold << std::endl;
	...
	static short floatToHalfFloat(float floatValue)
	{
		...
		else
		{
			...
			short output = short((value & FLOAT_SIGNIFICAND_MASK) >> 13);

			register int exponent = ((value & FLOAT_EXPONENT_MASK) >> 23);
			...
			if (exponent != 0)
			{
				...
				if (exponent > 142)
					return short((value & FLOAT_SIGN_MASK) >> 16) | short(0x7C00);
			...
			// Add the sign bit.
			output |= short((value & FLOAT_SIGN_MASK) >> 16);

			return output;
		}
	}

	static float halfFloatToFloat(short halfFloat)
	{
		...
		int value = int(halfFloat & HALF_FLOAT_SIGNIFICAND_MASK) << 13;

		register int exponent = int(halfFloat & HALF_FLOAT_EXPONENT_MASK) >> 10;
		...
		// Add the sign bit.
		value |= int(halfFloat & HALF_FLOAT_SIGN_MASK) << 16;

		return *((float*)&value);
	}

	void load_from_rendering_client(vertex_type* vertex_data, int* index_data, int vertexCount, int triCount)
	{
		//std::ofstream myfile("D:\\meshSimplification.txt");
		vertices.clear();
		normals.clear();
		triangles.clear();
	//	myfile << vertexCount << std::endl;
		const int modifiedVertexCount = vertexCount * 3 * 2;
		for (int i = 0; i < modifiedVertexCount; i += 6) // 3 position then 3 normals
		{
			Vertex v;
			v.p.x = static_cast<double>(halfFloatToFloat(vertex_data[i]));
			v.p.y = static_cast<double>(halfFloatToFloat(vertex_data[i + 1]));
			v.p.z = static_cast<double>(halfFloatToFloat(vertex_data[i + 2]));

			vec3f normal;
			normal.x = static_cast<double>(halfFloatToFloat(vertex_data[i + 3]));
			normal.y = static_cast<double>(halfFloatToFloat(vertex_data[i + 4]));
			normal.z = static_cast<double>(halfFloatToFloat(vertex_data[i + 5]));
			//myfile << "v " << v.p.x << " " << (v.p.y) << " " << (v.p.z) << std::endl;
			vertices.push_back(v);
			normals.push_back(normal);
		}
		const int indexCount = triCount * 3;
		for (int i = 0; i < indexCount; i += 3)
		{
			Triangle t;
			t.v[0] = index_data[i];
			t.v[1] = index_data[i + 1];
			t.v[2] = index_data[i + 2];

			//myfile << "t" << t.v[0] << " " << t.v[1] << " " << t.v[2] << std::endl;
	...
	void write_obj(const char* filename)
	{
		...
		loopi(0, normals.size())
		{
			fprintf(file, "vn %g %g %g\n", normals[i].x, normals[i].y, normals[i].z); //more compact: remove trailing zeros
		}
	...
	void fetch_mesh(vertex_type* vertex_data, int* index_data, int* nVertices, int* nTriangles)
	{
		//std::ofstream myfile("D:\\meshSimplification.txt", std::ofstream::out | std::ofstream::app);

		*nVertices = vertices.size();
		//myfile << "Mesh vSize =" << *nVertices << std::endl;
		for (int i = 0; i < *nVertices; ++i)
		{
			vertex_data[6 * i + 0] = floatToHalfFloat(static_cast<float>(vertices[i].p.x));
			vertex_data[6 * i + 1] = floatToHalfFloat(static_cast<float>(vertices[i].p.y));
			vertex_data[6 * i + 2] = floatToHalfFloat(static_cast<float>(vertices[i].p.z));
			//normal
			vertex_data[6 * i + 3] = floatToHalfFloat(static_cast<float>(normals[i].x));
			vertex_data[6 * i + 4] = floatToHalfFloat(static_cast<float>(normals[i].y));
			vertex_data[6 * i + 5] = floatToHalfFloat(static_cast<float>(normals[i].z));
		}

		*nTriangles = triangles.size();
		//myfile << "Mesh tSize =" << *nTriangles << std::endl;
		for (int i = 0; i < *nTriangles; ++i)
		{
			if (!triangles[i].deleted)
			{
				index_data[3 * i + 0] = triangles[i].v[0];
				index_data[3 * i + 1] = triangles[i].v[1];
				index_data[3 * i + 2] = triangles[i].v[2];
			}
		}
		//myfile << "Mesh Fetch End" << std::endl;
	}
};
///////////////////////////////////////////
